...
while (True):
    ...
    # for point_id in range(len(pcd.points)):
        # pcd.points[point_id].transform(pose_graph.nodes[point_id].pose)
    pcd_combined += pcd
...
#                                    max_correspondence_distance_fine)
# # o3d.io.write_point_cloud(f"{path}/copy_of_fragment.ply", pcds)
pcd_combined_down = pcd_combined.voxel_down_sample(voxel_size=voxel_size)
o3d.io.write_point_cloud(f"{path}/multiway_registration.ply", pcd_combined_down)
# Close readers ---------------------------------------------------------------
rd_pv.close()
rd_lf.close()
rd_rf.close()
